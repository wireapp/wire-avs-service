...
static void frame_alloc(struct jbuf *jb, struct frame **f)
{
	...
	if (le) {
		...
		STAT_INC(n_overflow);
		info("jbuf(%p): drop 1 old frame seq=%u (total dropped %u)\n",
		     jb, f0->hdr.seq, jb->stat.n_overflow);
...
int jbuf_alloc(struct jbuf **jbp, uint32_t min, uint32_t max)
{
	...
	for (i=0; i<jb->max; i++) {
		...
		list_append(&jb->pooll, &f->le, f);
		info("jbuf(%p): alloc: adding to pool list %u\n",
		     jb, i);
	}
...
int jbuf_put(struct jbuf *jb, const struct rtp_header *hdr, void *mem)
{
	...
	if (jb->running) {
		...
		if (seq_less((seq + jb->n), jb->seq_put)) {
			STAT_INC(n_late);
			info("jbuf(%p): put: packet too late: seq=%u "
			     "(seq_put=%u)\n",
			     jb, seq, jb->seq_put);
			return ETIMEDOUT;
		}
	...
	*/
	...
	for (le = tail; le; le = le->prev) {
		...
		if (seq_less(seq_le, seq)) { /* most likely */
			info("jbuf(%p): put: out-of-sequence"
			...
			/* Detect duplicates */
			info("jbuf(%p): put: duplicate: seq=%u\n", jb, seq);
			STAT_INC(n_dups);
			list_insert_after(&jb->framel, le, &f->le, f);
			frame_deref(jb, f);
			return EALREADY;
	     ...
	     void **mem, void *arg)
{
	...
	if (jb->seq_get) {
		...
		if (seq_less(f->hdr.seq, jb->seq_get)) {
			warning("jbuf(%p): get: seq=%u too late\n",
				jb, f->hdr.seq);
		}
		else if (seq_diff > 1) {
			STAT_ADD(n_lost, 1);
			info("jbuf(%p): get: n_lost: diff=%d,"
			     "seq=%u,seq_get=%u\n",
...
void jbuf_flush(struct jbuf *jb)
{
	...
	if (jb->framel.head) {
		info("jbuf(%p): flush: %u frames\n", jb, jb->n);
	}

	/* put all buffered frames back in free list */
	for (le = jb->framel.head; le; le = jb->framel.head) {
		info("jbuf(%p): flush: frame: seq=%u\n",
		     jb, ((struct frame *)(le->data))->hdr.seq);
